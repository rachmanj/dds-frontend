...
export default function AdditionalDocumentsPage() {
  ...
  const [deletingDocument, setDeletingDocument] =
    useState<AdditionalDocument | null>(null);
  ...
  const handleImport = async () => {
    ...
    setImporting(true);
    try {
      const result = await importAdditionalDocuments(
        importFile,
        checkDuplicates
      );
      ...
      if (result.success && result.data) {
        ...
        toast.success(
          `Import completed! ${result.data.imported} documents imported successfully.`
        );
    ...
    () => [
      ...
      },
      {
        ...
        cell: ({ row }) => {
          ...
          const isInTransit = document.is_in_transit || false;

          if (!isInTransit) {
            return <Badge variant="secondary">Available</Badge>;
          }

          const status = document.active_distribution?.status;
          const statusConfig = {
            verified_by_sender: {
          ...
          return (
            <Badge
              ...
              title={`Distribution: ${document.active_distribution?.distribution_number}`}
            >
              {config?.label || status}
            </Badge>
          );
          ...
          return (
            <div className="flex items-center justify-end space-x-2">
              <Button
                ...
                title={
                  isInTransit
                    ? `Cannot edit: Document is in transit (${document.active_distribution?.status})`
                    : "Edit document"
                }
              >
              ...
              <Button
                ...
                title={
                  isInTransit
                    ? `Cannot delete: Document is in transit (${document.active_distribution?.status})`
                    : "Delete document"
                }
              >
            ...
            <div className="text-center space-y-4">
              ...
              <div>
                ...
                <p className="text-muted-foreground">
                  You need to be logged in to access the additional documents
                  page.
                </p>
              </div>
        ...
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              {deletingDocument?.document_number}&rdquo;? This action cannot be
              undone.
            </DialogDescription>
          </DialogHeader>
          {deletingDocument && (
            <div className="py-4">
              <div className="space-y-2 text-sm">
                <p>
                  ...
                  {deletingDocument.document_number}
                </p>
                <p>
                  <strong>Type:</strong>{" "}
                  {deletingDocument.type?.type_name || "N/A"}
                </p>
                <p>
                  <strong>Document Date:</strong>{" "}
                  {formatDate(deletingDocument.document_date)}
                </p>
                <p>
                  <strong>PO Number:</strong> {deletingDocument.po_no || "N/A"}
                </p>
                <p>
                  <strong>Current Location:</strong>{" "}
                  {deletingDocument.cur_loc
                    ? getDepartmentDisplayName(deletingDocument.cur_loc)
                    : "N/A"}
                </p>
              </div>
            </div>
      ...
      <Dialog open={isImportDialogOpen} onOpenChange={setIsImportDialogOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Upload an Excel file (.xlsx, .xls) or CSV file containing ITO
              document data.
            </DialogDescription>
          </DialogHeader>
          ...
          {!importResult ? (
            <div className="space-y-4">
              ...
              <div className="flex items-center space-x-2">
                <Checkbox
                  ...
                  onCheckedChange={(checked) =>
                    setCheckDuplicates(checked as boolean)
                  }
                  disabled={importing}
                />
              ...
              <Alert>
                ...
                <AlertDescription>
                  <strong>Required columns:</strong> ito_no
                  <br />
                  <strong>Optional columns:</strong> ito_date, po_no, project,
                  receive_date, ito_remarks, ito_created_by, grpo_no,
                  origin_whs, destination_whs
                </AlertDescription>
              </Alert>
