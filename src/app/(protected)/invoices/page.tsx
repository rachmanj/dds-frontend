...
export default function InvoicesPage() {
    ...
    const availableProjectsForCreate = useMemo(() => {
        ...
        if (selectedSupplier.payment_project) {
            return projects.filter(project => project.code === selectedSupplier.payment_project);
        }
    ...
    const getSupplierDisplayName = useCallback((supplierId: number) => {
    ...
    const getSupplierOptionDisplay = (supplier: { name: string; sap_code?: string; city?: string }) => {
    ...
    const validateInvoiceNumber = useCallback((invoiceNumber: string, supplierId: number, excludeId?: number) => {
        if (!invoiceNumber || !supplierId) return true;

        const existingInvoice = invoices.find(invoice =>
            invoice.invoice_number.toLowerCase() === invoiceNumber.toLowerCase() &&
    ...
    const handleInvoiceNumberBlur = (invoiceNumber: string, supplierId?: number) => {
        const supplierIdToUse = supplierId || formData.supplier_id;

        if (invoiceNumber && supplierIdToUse) {
            const isValid = validateInvoiceNumber(invoiceNumber, supplierIdToUse, editingInvoice?.id);
            if (!isValid) {
                setValidationErrors(prev => ({
                    ...prev,
                    invoice_number: "This invoice number already exists for the selected supplier."
                }));
            } else {
                setValidationErrors(prev => {
                    const newErrors = { ...prev };
    ...
    const handleCreateInvoice = async () => {
        ...
        if (!validateInvoiceNumber(formData.invoice_number, formData.supplier_id)) {
            setValidationErrors(prev => ({
                ...prev,
                invoice_number: "This invoice number already exists for the selected supplier."
            }));
    ...
    const handleUpdateInvoice = async () => {
        ...
        // Validate invoice number
        if (!validateInvoiceNumber(formData.invoice_number, formData.supplier_id, editingInvoice.id)) {
            setValidationErrors(prev => ({
                ...prev,
                invoice_number: "This invoice number already exists for the selected supplier."
            }));
                                    ...
                                    {table.getRowModel().rows?.length ? (
                                        ...
                                        <TableRow>
                                            <TableCell colSpan={columns.length} className="text-center py-8">
                                                {globalFilter ? "No invoices found matching your search." : "No invoices found. Create your first invoice!"}
                                            </TableCell>
                                        </TableRow>
                                    )}
                                </TableBody>
                    ...
                    {deletingInvoice && (
                        <div className="py-4">
                            <div className="space-y-2 text-sm">
                                ...
                                <p><strong>Supplier:</strong> {deletingInvoice.supplier?.name || getSupplierDisplayName(deletingInvoice.supplier_id)}</p>
                                <p><strong>Type:</strong> {deletingInvoice.type?.type_name || "N/A"}</p>
                                <p><strong>Amount:</strong> {formatCurrency(deletingInvoice.amount, deletingInvoice.currency)}</p>
                                <p><strong>Invoice Date:</strong> {formatDisplayDate(deletingInvoice.invoice_date)}</p>
                                <p><strong>Status:</strong> {deletingInvoice.status.toUpperCase()}</p>
                            </div>
                        </div>
                    )}
                    <DialogFooter>
