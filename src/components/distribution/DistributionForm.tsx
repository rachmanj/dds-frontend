...
import {
  ...
  AdditionalDocument,
} from "@/lib/api/additional-documents";
...
interface DistributionFormProps {
  ...
  onSubmit: (
    data: CreateDistributionRequest | UpdateDistributionRequest
  ) => Promise<void>;
  onCancel: () => void;
  isEditing?: boolean;
  currentUser?: {
...
export function DistributionForm({
  ...
  const form = useForm<FormData>({
    ...
    defaultValues: {
      ...
      origin_department_id:
        initialData?.origin_department_id ||
        currentUser?.department_id ||
        undefined,
      destination_department_id:
        initialData?.destination_department_id || undefined,
  ...
  const [selectedDocuments, setSelectedDocuments] = useState<
    SelectedDocument[]
  >([]);
  const [autoIncludedDocuments, setAutoIncludedDocuments] = useState<
    SelectedDocument[]
  >([]);
  const [warnings, setWarnings] = useState<DistributionWarning[]>([]);
  const [availableInvoices, setAvailableInvoices] = useState<Invoice[]>([]);
  const [availableAdditionalDocs, setAvailableAdditionalDocs] = useState<
    AdditionalDocument[]
  >([]);
  ...
  const filteredOriginDepartments = sortedDepartments.filter(
    (dept) =>
  ...
  const filteredDestinationDepartments = sortedDepartments.filter(
    (dept) =>
      dept.name.toLowerCase().includes(destinationDeptSearch.toLowerCase()) ||
      dept.project
        .toLowerCase()
        .includes(destinationDeptSearch.toLowerCase()) ||
  ...
  const loadAvailableDocuments = useCallback(async () => {
    ...
    try {
      if (documentType === "invoice") {
        const invoices = await invoiceService.getForDistribution({
        ...
        const additionalDocs =
          await additionalDocumentService.getForDistribution({
            search: documentSearch,
          });
  ...
  const addDocument = async (
    ...
    if (!isAlreadySelected) {
      ...
      if (isEditing && initialData) {
        try {
          await distributionService.attachDocuments(initialData.id, [
            { type, id: document.id },
          ]);
  ...
  const removeDocument = async (type: DocumentType, id: number) => {
    ...
    if (isEditing && initialData) {
      try {
        ...
        toast.success(
          `${type === "invoice" ? "Invoice" : "Document"} removed successfully`
        );
      } catch (error: any) {
  ...
  React.useEffect(() => {
    console.log("Form Debug Info:", {
      ...
      autoIncludedDocumentsCount: autoIncludedDocuments.length,
      allDocumentsCount: allDocuments.length,
      documentType,
      formValid: form.formState.isValid,
      formErrors: form.formState.errors,
      buttonDisabled:
        isSubmitting || (!isEditing && selectedDocuments.length === 0),
    });
  }, [
    isSubmitting,
    isEditing,
                      ...
                      <FormControl>
                        ...
                        >
                            ...
                            >
                              ...
                              <div>
                                ...
                                <div className="text-sm text-muted-foreground">
                                  Distribute invoices (attached additional
                                  documents will be auto-included)
                                </div>
                              </div>
                            </label>
                          </div>
                          <div className="flex items-center space-x-2">
                            ...
                            />
                            <label
                              htmlFor="additional_document"
                              className="flex items-center space-x-2 cursor-pointer"
                            >
                              <FileText className="h-4 w-4 text-green-500" />
                              <div>
                                <div className="font-medium">
                                  Additional Documents Only
                                </div>
                                <div className="text-sm text-muted-foreground">
                        ...
                        <div className="w-full p-3 bg-muted/50 border rounded-md">
                          <div className="flex items-center justify-between">
                            <div>
                              ...
                              <div className="text-sm text-muted-foreground">
                                Your Department -{" "}
                                {currentUser.department.location_code}
                              </div>
                            </div>
                            ...
                            <div className="p-2 border-b">
                              <div className="relative">
                                ...
                                <Input
                                  ...
                                  onChange={(e) =>
                                    setOriginDeptSearch(e.target.value)
                                  }
                                  className="pl-8 h-8"
                                />
                              </div>
                        ...
                        <div className="p-2 border-b">
                          <div className="relative">
                            ...
                            <Input
                              ...
                              onChange={(e) =>
                                setDestinationDeptSearch(e.target.value)
                              }
                              className="pl-8 h-8"
                            />
                          </div>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormDescription>
                      Additional information about this distribution (max 1000
                      characters)
                    </FormDescription>
                    <FormMessage />
                  </FormItem>
                  ...
                  {isEditing
                    ...
                    : `Choose ${
                        documentType === "invoice"
                          ? "invoices"
                          : "additional documents"
                      } to include in this distribution`}
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                {/* Search */}
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <Badge variant="outline">
                      {documentType === "invoice"
                        ...
                        : availableAdditionalDocs.length}{" "}
                      available
                    </Badge>
                  </div>
                      ...
                      {documentType === "invoice" ? (
                        availableInvoices.length === 0 ? (
                          ...
                          availableInvoices.map((invoice) => (
                            <div
                              ...
                              onClick={async () =>
                                await addDocument("invoice", invoice)
                              }
                            >
                              <div className="flex items-center justify-between">
                                <div className="flex-1">
                                  ...
                                  {invoice.has_location_mismatch && (
                                    <div className="text-xs text-amber-600 mt-1">
                                      ⚠️ Some attached documents have location
                                      mismatch
                                    </div>
                                  )}
                                </div>
                                <Button
                                  ...
                                  variant="outline"
                                  size="sm"
                                  disabled={selectedDocuments.some(
                                    (doc) =>
                                      doc.type === "invoice" &&
                                      doc.id === invoice.id
                                  )}
                        ...
                        availableAdditionalDocs.map((doc) => (
                          <div
                            ...
                            className="p-3 border-b last:border-b-0 hover:bg-muted/50 cursor-pointer"
                            onClick={async () =>
                              await addDocument("additional_document", doc)
                            }
                          >
                            <div className="flex items-center justify-between">
                              <div className="flex-1">
                    ...
                    >
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          ...
                          <div className="text-sm text-muted-foreground">
                            {doc.type === "invoice" ? (
                              <>
                                {doc.data.supplier?.name} •{" "}
                                {formatDate(doc.data.invoice_date)}
                              ...
                              <>
                                {doc.data.type?.type_name} •{" "}
                                {formatDate(doc.data.document_date)}
                                {doc.data.remarks && (
                                  <span> • {doc.data.remarks}</span>
                                )}
                              </>
                            )}
                          </div>
                      ...
                      {!doc.auto_included && (
                        <Button
                          ...
                          size="sm"
                          onClick={async () =>
                            await removeDocument(doc.type, doc.id)
                          }
                        >
                          <X className="h-4 w-4" />
                        </Button>
              ...
              <AlertDescription>
                <div className="space-y-2">
                  ...
                  <div className="text-sm">
                    {autoIncludedDocuments.length} additional document(s) will
                    be automatically included with the selected invoices.
                  </div>
                </div>
              </AlertDescription>
            </Alert>
              ...
              <Button
                ...
                onClick={(e) => {
                  ...
                  if (
                    ...
                    e.preventDefault();
                    console.log("Button click prevented - disabled state");
                  }
                }}
              >
                {isSubmitting
